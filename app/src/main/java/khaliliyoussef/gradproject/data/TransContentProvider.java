...
public class TransContentProvider extends ContentProvider {
    ...
    private TransDbHelper mTransDbHelper;

    /* onCreate() is where you should initialize anything you’ll need to setup
    your underlying data source.
    In this case, you’re working with a SQLite database, so you’ll need to
    initialize a TransDbHelper to gain access to it.
     */
    @Override
    public boolean onCreate() {
        // Complete onCreate() and initialize a TaskDbhelper on startup
        // [Hint] Declare the TransDbHelper as a global variable

        Context context = getContext();
        mTransDbHelper = new TransDbHelper(context);
        return true;
    }
    ...
    public Uri insert(@NonNull Uri uri, ContentValues values) {
        ...
        switch (match) {
            case WORDS:
                ...
                if ( id > 0 ) {
                    returnUri = ContentUris.withAppendedId(TransContract.TaskEntry.CONTENT_URI, id);
                } else {
    ...
    public Cursor query(@NonNull Uri uri, String[] projection, String selection,
        ...
        final SQLiteDatabase db = mTransDbHelper.getReadableDatabase();
